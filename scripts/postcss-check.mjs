...
const cases = [
  ...
  { css: injected, label: 'postcss · composed, registering', plugins: () => [jumi({ plugin: jumiBundle })] },
  // The same stylesheet as a project that writes the directive itself.
  { css: registered, label: 'postcss · composed, explicit', plugins: () => [jumi({ plugin: jumiBundle })] },
  { css: registered, label: 'postcss · finalizer after Tailwind', plugins: () => [tailwind({ base: dir }), jumiFinalizer()] },
  { css: registered, label: 'postcss · finalizer before Tailwind', plugins: () => [jumiFinalizer(), tailwind({ base: dir })] },
  { css: registered, label: 'postcss · composed, optimize: false', plugins: () => [jumi({ tailwind: { optimize: false } })] },
]
...
for (const test of cases) {
  const result = await postcss(test.plugins()).process(test.css, { from: path.join(dir, 'in.css') })
...
const plain = await postcss([jumi({ plugin: jumiBundle })]).process('.card { color: red; }', { from: path.join(dir, 'plain.css') })

const claims = [
  {
    actual: registered1 === explicit,
    detail: 'the directive Jumi adds and the directive an author writes compile identically',
    label: 'equivalent',
  },
  { actual: !plain.css.includes('@plugin'), detail: 'a stylesheet that is not an entrypoint is untouched', label: 'untouched' },
]

console.log('')

for (const claim of claims) {
  console.log(`    ${claim.actual ? '✓' : '✗'} ${claim.label.padEnd(12)}${claim.detail}`)
  if (!claim.actual) failures.push(`registration · ${claim.label}: ${claim.detail}`)
}
...
console.log('  the finalizer works on either side of it, and no publication reaches the stylesheet.\n')

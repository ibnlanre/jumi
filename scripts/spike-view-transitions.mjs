...
for (const name of ['cross-from.html', 'cross-to.html']) {
  routes.set(`/${name}`, (url) => {
    if (url.searchParams.has('sheet') || url.searchParams.has('sheets') || url.searchParams.has('types')) {
    ...
    const sheet = name === 'cross-to.html' ? variant.sheetTo ?? variant.sheet : variant.sheet
 ...
 * present and correct.
 */
const declarationsOf = rule => (rule.nodes ?? [])
  .filter(node => node.type === 'decl')
  .map(node => `${node.prop}: ${node.value};`)
  .join(' ')

const substrateRule = sheetRules.find(rule => rule.nodes?.some(node =>
  node.type === 'decl' && node.prop === '--jumi-animation-name' && node.value === 'none'))
...
const compositionRule = sheetRules.find((rule) => {
  ...
  const lastReset = Math.max(props.indexOf('animation-composition'), props.indexOf('animation-timeline'))
...
const activationRule = selector => sheetRules.find(rule => rule.selector === selector
  && rule.nodes?.some(node => node.type === 'decl' && node.prop.startsWith('--jumi-slot-')))
...
let jumiRules = {
  activations: Object.fromEntries(['.animate-fade-in', '.animate-bounce-in'].map(selector =>
    [selector, activationRule(selector) ? declarationsOf(activationRule(selector)) : null])),
  aggregate: extracted.aggregate,
  hoistedRules: sheetRules.filter(rule => rule.nodes?.some(node =>
    node.type === 'decl' && node.prop.startsWith('--jumi-slot-'))).length,
...
const activationOf = selector => jumiRules.activations[selector] ?? '/* ACTIVATION NOT EXTRACTED */'
...
await probe('P3a', 'Can a utility on the source element reach the pseudo tree', async () => {
  ...
  return {
    extracted: {
      activationBytes: Object.fromEntries(Object.entries(jumiRules.activations)
        .map(([selector, value]) => [selector, value?.length ?? 0])),
      aggregateAnimationHead: (jumiRules.aggregate?.match(/(?<![\w-])animation:[^;]{0,80}/) ?? [''])[0],
      ...
      aggregateDeclares: compositionRule
        ? (compositionRule.nodes ?? []).filter(node => node.type === 'decl').map(node => node.prop)
        : [],
      aggregateSelectors: compositionRule ? compositionRule.selectors.length : 0,
      aggregateSlotRefs: (jumiRules.aggregate?.match(/var\(--jumi-slot-/g) ?? []).length,
      hoistedRules: jumiRules.hoistedRules,
      keyframesInSnapshot: keyframeCount,
      substrateAnimationHead: (jumiRules.substrate?.match(/(?<![\w-])animation:[^;]{0,80}/) ?? [''])[0],
      substrateBytes: jumiRules.substrate?.length ?? 0,
      substrateDeclares: substrateRule
        ? (substrateRule.nodes ?? []).filter(node => node.type === 'decl').map(node => node.prop).length
        : 0,
...
await probe('P3b', 'The emitted Jumi composition, replayed verbatim under a view-transition selector', async () => {
  ...
  const applied = await lab(() => {
    ...
    const heroOld = rules.find(rule => (rule.selectorText ?? '').includes('view-transition-old(hero)'))
    const text = heroOld?.style.cssText ?? ''
    const at = text.indexOf('animation:')
    const cs = getComputedStyle(document.documentElement, '::view-transition-old(hero)')

    return {
      animationAt: at,
      animationDeclared: [...(heroOld?.style ?? [])].filter(name => name.startsWith('animation')),
...
async function crossDocument({ label, sheet, sheets, types = '' }) {
  ...
  const linked = await fresh.evaluate(() => [...document.styleSheets].map(sheet => sheet.href))
 ...
 * markers.
 */
const markerMap = run => Object.fromEntries(Object.entries(run.ready?.pseudos ?? {})
 ...
 */
const controlReads = control => control.raw === 'yes' && control.registered === 'yes' && control.root === 'yes'

await probe('P18', 'Cross-document: which document styles the pseudo tree', async () => {
  /**
  ...
  const control = await (async () => {
    ...
    const read = await lab(() => {
      const get = (pseudo, name) => getComputedStyle(document.documentElement, pseudo).getPropertyValue(name).trim()

      return {
        liveHeroOld: document.getAnimations().some(a => a.effect?.pseudoElement === '::view-transition-old(hero)'),
        liveRootOld: document.getAnimations().some(a => a.effect?.pseudoElement === '::view-transition-old(root)'),
        raw: get('::view-transition-old(hero)', '--probe-ctl-raw'),
        registered: get('::view-transition-old(hero)', '--probe-ctl-registered'),
  ...
  const forward = await crossDocument({ label: 'destination links /own-to.css', sheets: '/own-from.css,/own-to.css' })
  const swapped = await crossDocument({ label: 'destination links /own-from.css', sheets: '/own-to.css,/own-from.css' })
...
async function crossFade({ blend = null, label, style }) {
  ...
  const started = await lab(({ op }) => window.__vt.start(op), { op: 'recolor' })
  const measured = await lab(({ pseudos }) => window.__vt.measure(pseudos), { pseudos: HERO_PSEUDOS })
  const durations = measured.animations.map(animation => animation.duration).filter(value => typeof value === 'number')
  ...
  const at = async (t) => {
    const state = await lab(({ pseudos, t }) => window.__vt.advance(t, pseudos), { pseudos: HERO_PSEUDOS, t })
...
await probe('P19', 'The image pair: does overriding the cross-fade bleed the backdrop through', async () => {
  ...
  return {
    ...
    jumi: await crossFade({ label: 'jumi composition on old/new', style: crossFadeCss() }),
    jumiBlend: await crossFade({ label: 'jumi composition + mix-blend-mode: plus-lighter', style: crossFadeCss({ blend: 'plus-lighter' }) }),
 ...
 */
await probe('P20', 'prefers-reduced-motion: which emitted shape actually stops the animation', async () => {
  const composition = `
    ::view-transition-old(hero), ::view-transition-new(hero) {
      ${jumiRules.substrate}
      ${jumiRules.aggregate}
  ...
  for (const preference of ['no-preference', 'reduce']) {
    const own = await browser.newContext({ reducedMotion: preference, viewport: { height: 600, width: 900 } })
    ...
    const on = {
      advance: (t, pseudos) => ownPage.evaluate(({ pseudos, t }) => window.__vt.advance(t, pseudos), { pseudos, t }),
      measure: pseudos => ownPage.evaluate(({ pseudos }) => window.__vt.measure(pseudos), { pseudos }),
      mount: items => ownPage.evaluate(({ items }) => window.__vt.mount(items), { items }),
      start: op => ownPage.evaluate(({ op }) => window.__vt.start(op), { op }),
      style: css => ownPage.evaluate(({ css }) => window.__vt.style(css), { css }),
    }

    const measured = {}
    const matches = await ownPage.evaluate(() => matchMedia('(prefers-reduced-motion: reduce)').matches)
    ...
    for (const [name, css] of Object.entries(shapes)) {
      ...
      measured[name] = {
        ...
        names: driven.animations.map(animation => `${animation.pseudo} → ${animation.name}`),
        newName: names(driven.pseudos['::view-transition-new(hero)']?.animationName ?? ''),
        ran: driven.animations.some(animation => String(animation.name ?? '').startsWith('jumi-')),
      }
...
const IDENTITIES = [
  ...
  ['--foo', 'leading double hyphen — a valid custom-ident, though it reads like a custom property'],
  ['HERO', 'uppercase'],
  ...
  ['hero)', 'contains a closing paren — would break the selector by concatenation'],
  ['hero{', 'contains a brace'],
  ['a b', 'contains a space'],
]

await probe('P21', 'identity serialization: which values are a usable transition identity', async () => {
  const rows = []

  for (const [identity, note] of IDENTITIES) {
    const declared = await lab(({ value }) => ({
      ...
      asPseudo: CSS.supports('selector(::view-transition-old(' + value + '))'),
    }), { value: identity })

    await lab(({ name }) => window.__vt.mount([{ id: 'hero', name, text: 'hero' }]), { name: identity })

    const started = await lab(({ op }) => window.__vt.start(op), { op: 'move' })
    const built = await lab(() => ({
      // A group for a *named* participant only exists if the name took effect.
      group: [...new Set(document.getAnimations()
        .map(animation => animation.effect?.pseudoElement)
        .filter(pseudo => pseudo?.startsWith('::view-transition-group(')))],
      old: [...new Set(document.getAnimations()
        .map(animation => animation.effect?.pseudoElement)
    ...
    const named = built.group.filter(pseudo => pseudo !== '::view-transition-group(root)')
 ...
 */
await probe('P22', 'does a wrapped identity cost the browser shared-element travel', async () => {
  const reduced = await browser.newContext({ reducedMotion: 'reduce', viewport: { height: 600, width: 900 } })
  const reducedPage = await reduced.newPage()
  ...
  const on = {
    advance: (t, pseudos) => reducedPage.evaluate(({ pseudos, t }) => window.__vt.advance(t, pseudos), { pseudos, t }),
    measure: pseudos => reducedPage.evaluate(({ pseudos }) => window.__vt.measure(pseudos), { pseudos }),
    mount: items => reducedPage.evaluate(({ items }) => window.__vt.mount(items), { items }),
    start: op => reducedPage.evaluate(({ op }) => window.__vt.start(op), { op }),
    style: css => reducedPage.evaluate(({ css }) => window.__vt.style(css), { css }),
  }
  ...
  for (const [name, css] of Object.entries(shapes)) {
    ...
    const groupOf = state => Object.entries(state.pseudos)
      .find(([pseudo]) => pseudo === '::view-transition-group(hero)')?.[1]
    ...
    measured[name] = {
      ...
      groupBuilt: early.animations.some(animation => animation.pseudo === '::view-transition-group(hero)'),
   ...
   */
  const narrow = await browser.newContext({ viewport: { height: 600, width: 500 } })
  ...
  // nothing.
  const narrowMatched = await narrowPage.evaluate(() => matchMedia('(width >= 40rem)').matches)

  await narrowPage.evaluate(({ op }) => window.__vt.start(op), { op: 'move' })

  const narrowBuilt = await narrowPage.evaluate(({ pseudos, t }) => window.__vt.advance(t, pseudos), {
  ...
  return {
    ...
    smUnmatched: {
      groupBuilt: narrowBuilt.animations.some(a => a.pseudo === '::view-transition-group(hero)'),

...
const brand = owner => value => ({ '--probe-owner': owner, '--probe-value': value })

const phraseRoute = (owner, inner) => (value, extra) => (phraseOf(value) ? inner(value, extra) : {})

export default plugin((api) => {
  // a · the status quo: one typed handler, so a phrase never arrives.
  api.matchUtilities({ 'probe-a': brand('typed') }, { type: 'length', values: {} })
  ...
  api.matchUtilities({ 'probe-b': brand('typed') }, { type: 'length', values: {} })
  api.matchUtilities({ 'probe-b': phraseRoute('phrase', brand('phrase')) }, { type: 'any', values: {} })
  api.matchUtilities({ 'probe-b2': phraseRoute('phrase', brand('phrase')) }, { type: 'any', values: {} })
  api.matchUtilities({ 'probe-b2': brand('typed') }, { type: 'length', values: {} })
  ...
  api.matchUtilities({ 'probe-d': brand('typed') }, { type: 'length', values: { 100: '100px' } })
  api.matchUtilities({ 'probe-d': phraseRoute('phrase', brand('phrase')) }, { type: 'any', values: {} })
})

 ...
 *                only thing that builds it. Nine of the fifteen stages below used to run `pnpm run bundle`
 ...
 *   everything   after those two, independent: fifteen stages that share nothing but the tree they read
 *
 * Those first two run alone and stream their output, because they are the root of every other stage and their
 * logs are worth watching live. The remaining fifteen run through a **pool** (`--jobs`, default 4) with their
 * output buffered per stage and printed as each one finishes: interleaving fifteen streams would make a failure
 ...
 *
 * The buffer is the reason a pooled run stays readable: fifteen streams writing to one terminal interleave, and

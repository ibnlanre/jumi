...
- **Family / visual:** repeated apertures. Multiple parallel strips reveal the _same stationary surface_ together, leaving alternating visible and hidden regions during the transition.
...
“Complete” means no additional _semantic preset_ earned a recommendation from the sources examined. It does not mean all possible curves or combinations exist, or that existing quality was tested.
...
launch _loudly and by name_, records it in `results.json` (`engines.ran` / `engines.skipped`) and fails

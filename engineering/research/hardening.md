...
The scanner extracts backticked spans that are _entirely_ a class-like token, builds them, and reports the ones
...
That split is the point of the section: a record _should_ contain class names that resolve to nothing when the
...
| where the release version comes from                                                | `package.json` — so the bump has to happen _before_ generation                                                   |
...
- **`-r 0` overwrites the whole file** rather than prepending. This is not theoretical: a stray inspection run of
  mine rewrote `CHANGELOG.md` and destroyed the hand-written notes, and the damage was found by _content_, not by
...
Four ways this audit was wrong before it was right, all of them producing _false findings_ — the expensive
...
- **A catalogue left out.** The effects check demanded the docs name 216 effects that _are_ documented, because
...
- **A guard check that spanned blocks.** The first version of §7 asked the stylesheet _text_ whether a feature
...
the manifest stores `node scripts/x.mjs`, so _every_ script looked unregistered. The check parses the command

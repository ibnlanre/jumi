...
**The hypothesis holds, and the emission already implements it.** A scroll timeline is a _control
over an existing Jumi animation_, not a new animation family — and `animation-timeline` is already a
...
normalizes each animation to _its own effect end_: resolved `duration` is a percentage of the range
...
becomes a share of the scroll that also _compresses_ the motion. Full classification in §5.
...
documents write custom properties nothing reads (measured inert twice, §6). A _named_ timeline can be
...
.animate-fade-in {
  animation:
    var(--jumi-slot-rotate-3zWYd, none), var(--jumi-slot-fade-in, none);
  animation-composition:
    var(--jumi-rotate-animation-composition, …),
    var(--jumi-fade-in-animation-composition, …);
  animation-timeline:
    var(--jumi-rotate-animation-timeline, …),
    var(--jumi-fade-in-animation-timeline, …);
...
- **Per-slot scoping already works.** `animation-timeline-view/rotate` writes
  ...
  composition's position 0 reads it. The `/<slot>` vocabulary is the _same_ vocabulary the timing
...
- **The global control broadcasts.** `animation-timeline-scroll` (no modifier) writes
  `--jumi-animation-timeline`, the fallback of _every_ position, so a two-slot element gets two
...
- **The order after the shorthand is load-bearing.** `dOrderBefore` (timeline declared _before_ the
  ...
  `animation-composition: add` surviving the shorthand in _both_ orders — measured, the shorthand
...
climbing. Every arm below is the _same_ `animate-fade-in` — same keyframes, same slot, same
...
| `aScrollAxisX` | `scroll(x)`                   | _inactive_                 | — · — · — · — · —                         |
| `aViewAxisX`   | `view(x)`                     | _constant 60%_             | 0.6 · 0.6 · 0.6 · 0.6 · 0.6               |
| `aNone`        | `none`                        | _no animation_             | 0 · 0 · 0 · 0 · 0                         |
...
- **Inactive is silent.** `scroll(x)` on a page with no horizontal overflow yields _no animation at
  all_ and `view(x)` reports a constant — the element simply paints its base state. There is no
...
- **A view timeline reports outside its range.** Before entry, `currentTime` is _negative_
...
scroll-driven is scroll-driven _for that animation name_ — no inference from the list order is
...
| consume a _named_ timeline               | **works**                        | `bArbNamed` ← `--page` on `#pane`                                |
...
| `animation-delay`           | **meaningful, reinterpreted** | a share of the range _and_ a compression: resolved duration `83.3333%` at `delay 200ms/1s`, `62.5%` at `600ms/1s`     |
...
_(This section is the state before the implementation below — kept because it is the measurement the
change was justified by, not because it is still true.)_
...
1. **The property never reaches the element.** `dRangeJumi` (`animation-range-start-timeline-entry`,
   ...
   the _same_ progress series as each other — i.e. as a bare `view()`.
...
| `eNamedOutside` (outside the declaring subtree)       | `--page` | _(none)_         | —                      |
| `eNamedMissing` (a name nothing declares)             | `--nope` | _(none)_         | —                      |
...
A view timeline is usable by the declaring subject _and_ its descendants, which matters for the
...
- **The clamp is the author's, and the obvious spelling is a trap.** `motion-reduce:…-none` works
  (it removes the motion), but wrapping the _timeline_ in `motion-safe:` does not: under `reduce` the
...
## 10 · The separate track: scroll-_triggered_ animations
...
2. **Nothing else is required for the feature as measured.** No new animation family, no new class
   ...
   not have when it first said "free": see _The fallback is a decision_ below.
3. **Named-timeline _declaration_ stays unexposed.** Anonymous `scroll()`/`view()` cover the shipped
...
- **`normal 0% normal 100%` is not a legal `animation-range`.** It is what the existing variable
  ...
  _end_. Measured with a list that had one position set and one falling back: computed `normal`, i.e.
...
- **So the two timeline-name halves declare no default at all**, deliberately, and the composition
  ...
  measures _identically to `normal`_ on both a scroll and a view timeline (progress series equal to
...
- Reverting the composed default to `normal 0% normal 100%` failed the range arms at runtime — the
  live position resolved to `normal` and its progress became the scroll fraction. _7/10._
...
the author's concepts rather than the grammar's — which _scroller_, along which _axis_. The arbitrary
range forms — `animation-range-[entry_0%_cover_50%]`, `animation-range-start-[entry_25%]` — _are_ the
...
`animation-range-start-timeline-normal` wrote a _half_ of `normal` — and measured,
...
a _whole_ range, which is what the bare `animation-range` utility writes, so it stays in that set and
...
**The halves got simpler as a consequence.** A half used to be a _composition_ of a name and an
...
emitted — a silent refusal of the _common_ case, since the inset is a `<length-percentage>` and a
...
places _one_ of them and leaves its neighbours where they are:
...
- `docs/src/pages/docs/controls.md` claimed Jumi exposes `animation-range` controls while they wrote
  ...
  reader needs from this measurement pass: that a scroll-driven motion falls back to _document time_
...
scripts docs/src docs/astro.config.ts`, and `eslint.config.mts` ignored `**/scripts/**` — ESLint 9
  _errors_ when a path it is given is entirely ignored, so `lint` failed before checking anything and

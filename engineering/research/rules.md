...
| 6   | Structural CSS claims come from **parsed declarations**, not substring matches.                                                 | D.3.11 — a scan for `offset-position:` matched inside `--jumi-offset-position:` | owed — current callers avoid the defect by using bounded needles; structural declaration parsing is not yet enforced generally    |

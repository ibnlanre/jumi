...
- **Is the naming model coherent?** The _model_ is coherent — property name first, value last, one route per
  property — but **coverage has a hole in the `transform` family**: `animate-transform-origin-*` exists,
  `animate-perspective-*` and `animate-perspective-origin-*` do not, and the `perspective` _property_ is
...
- `--jumi-perspective` is a **part of the `transform` value** — `src/composition/transform.ts` joins it with
  the matrix, rotate, scale, skew and translate parts. So `perspective` is reachable _as a transform
  function_, under `animate-transform-[perspective(400px)]`, and not as a property.
...
reason (no registration), not because two naming schemes disagree. What is _not_ coherent is the coverage:
...
walking up to a source file that _names_ the property, which is a weaker fact than it looked: the two builds

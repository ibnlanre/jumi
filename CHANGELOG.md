...
behind the *transform function*; it is now `--jumi-perspective-3d`, and `--jumi-perspective` belongs to the
...
bare. A name that is *valid* but matches no motion is still silent: controls configure motion, they do not
create it.

...
import {
  ...
  rangeFromSelector,
  rangeReadings,
} from '@/helpers/carriers/animation-range'
...
describe('the range a selector encodes', () => {
  it('reads the range out of the class the author typed', () => {
    expect(rangeFromSelector('.animation-range-entry\\:animate-fade-in')).toBe(
      'entry',
    )
    expect(
      rangeFromSelector('.animation-range-exit-crossing\\:animate-fade-out'),
    ).toBe('exit-crossing')
  })
  ...
  it('finds it among stacked conditions, because it is not always first', () => {
    ...
    expect(
      rangeFromSelector(
        '.hover\\:animation-range-entry\\:animate-fade-in:hover',
      ),
    ).toBe('entry')
    expect(
      rangeFromSelector(
    ...
    expect(
      rangeFromSelector('.sm\\:animation-range-entry\\:animate-fade-in'),
    ).toBe('entry')
    expect(
      rangeFromSelector(
  ...
  it('does not confuse the utility for the variant', () => {
    ...
    expect(
      rangeFromSelector('.animation-range-\\[entry_0\\%_cover_50\\%\\]'),
    ).toBeNull()
...
describe('the ranges Jumi will write', () => {
  it('accepts the names, the keyword alone, and the offset shapes', () => {
    ...
    expect(
      [...accepted, ...lengths].filter(range => !rangeAccepted(range)),
    ).toEqual([])
  })
...
describe('what a rule says about its range', () => {
  ...
  it('reports a range it cannot attribute when an optimizer merged two motions into one rule', () => {
    ...
    expect(
      readings.every(reading => reading.slot === null && reading.motions === 2),
    ).toBe(true)
  })
})

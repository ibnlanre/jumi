...
export const propertyVariables: DependencyGraph = {
  ...
  'box-shadow': {
    ...
    // addresses either, so no phrase can write a frame key for them. That is what keeps them out of the
    // per-frame substitution in `propertyKeyframeValue`, which is made only for the components the
    // `surfaces` registry records — a read needs a writer **class**, and for these two there is none.
    // Recorded because they look exactly like the constituents that *are* authorable — `scale-x`,
    // `outline-width`, `backdrop-filter-blur` — and the difference is only visible in the candidate
    // set, not in this table.

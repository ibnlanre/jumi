...
describe("mergeTheme", () => {
  it("should merge multiple theme objects", () => {
    const theme1 = { color: "red", fontSize: 12 };
    const theme2 = { color: "blue", margin: 10 };
  ...
  it("should merge deeply only at the first level (shallow merge)", () => {
    const theme1 = { nested: { a: 1 }, color: "red" };
    const theme2 = { nested: { b: 2 }, color: "blue" };
    const result = mergeTheme(theme1, theme2);
    expect(result).toEqual({ nested: { b: 2 }, color: "blue" });
  });

  it("should not mutate input objects", () => {
    const theme1 = { color: "red" };
    const theme2 = { color: "blue" };
    const theme1Copy = { ...theme1 };
  ...
  it("should merge more than two themes", () => {
    ...
    const t2 = { b: 2 };
    const t3 = { c: 3 };
    expect(mergeTheme(t1, t2, t3)).toEqual({ a: 1, b: 2, c: 3 });
  });

  it("should handle themes with undefined values", () => {
    const t1 = { a: undefined, b: 2 };
    const t2 = { a: 1 };
    expect(mergeTheme(t1, t2)).toEqual({ a: 1, b: 2 });
  });
});

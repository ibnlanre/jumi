...
function paint(index: number, image: HTMLImageElement) {
  ...
  for (const key of cache.keys())
    if (Math.abs(key - index) > 8) cache.delete(key)
}
function sample() {
  raf = 0
  if (
  ...
  if (target !== painted) {
    if (cache.has(target)) paint(target, cache.get(target)!)
      ...
      const index = target
      void load(index)
        .then(image => paint(index, image))
        .catch(() => {
          canvas.dataset.mediaError = 'Frame unavailable'
        })
    }
  }
  for (const index of [target - 1, target + 1])
    if (index >= 0 && index < urls.length) void load(index).catch(() => {})
...
for (const button of document.querySelectorAll<HTMLButtonElement>(
  ...
  button.addEventListener('click', () => {
    for (const other of document.querySelectorAll('[data-chart]'))
      other.setAttribute('aria-pressed', String(other === button))
    for (const panel of document.querySelectorAll<HTMLImageElement>(
      '[data-chart-panel]',
    ))
...
for (const element of document.querySelectorAll('[data-entered]'))
